...
class ExportService:
    ...
    def generate_pdf(
        ...
        # Header
        story.append(Paragraph("🎯 Hikaru Dashboard Report", styles['Title']))
        story.append(Paragraph(f"<b>File:</b> {filename}", styles['Subtitle']))
        ...
        if global_summary:
            ...
            story.append(Paragraph(global_summary, styles['BodyText']))
        ...
        # Data Preview
        story.append(Paragraph("📊 Data Preview (First 10 Rows)", styles['Heading1']))
        if schema.preview:
            # Get column names
        ...
        story.append(Paragraph(f"📈 Generated Charts ({len(charts)})", styles['Heading1']))
        story.append(Spacer(1, 0.2*inch))

        for i, chart in enumerate(charts, 1):
            # Chart header
            story.append(Paragraph(f"Chart {i}: {chart.title}", styles['Heading2']))
            story.append(Paragraph(
            ...
            story.append(Paragraph(f"<b>Data:</b> {data_summary}", styles['BodyText']))

            # AI Insight
            if chart.insight:
                story.append(Spacer(1, 0.1*inch))
                insight_para = Paragraph(
                    f"<b>💡 AI Insight:</b> {chart.insight}",
                    styles['Insight']
                )
        ...
        # Footer
        story.append(PageBreak())
        story.append(Paragraph("Generated by Hikaru - AI Data Insight Board", styles['Footer']))
        story.append(Paragraph("Powered by Sanzoku Labs", styles['Footer']))
