...
async def compare_files(
    ...
    try:
        ...
        file_a = (
            db.query(FileModel)
            .filter(FileModel.id == comparison_data.file_a_id, FileModel.project_id == project_id)
            .first()
        )

        file_b = (
            db.query(FileModel)
            .filter(FileModel.id == comparison_data.file_b_id, FileModel.project_id == project_id)
            .first()
        )
        ...
        overlay_charts = comparison_service.generate_overlay_charts(
            df_a, df_b, file_a.filename, file_b.filename, comparison_data.comparison_type
        )
